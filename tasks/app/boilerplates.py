    ...
    for config_type in CONFIG_TYPES:
        ...
        if not setted_default:
            ...
            for key in RANDOM_KEYS:
                ...
                value = rlinput(
                    "请设置 %s (%s) \n(默认 %s): \n" % (key, HELPS[key], rand_str), rand_str
                )
        ...
        env = jinja2.Environment(
            loader=jinja2.FileSystemLoader("tasks/app/templates/configurations/", autoescape=True)
        )
        ...
        for template_name, conf_path in [
            ...
            template.stream(**config).dump("%s" % conf_path.format(config=config_type))
        ...
        for template_name, conf_path in [
            ...
            template.stream(
                db_username=u.username, db_password=u.password, db_name=u.database
...
def crud_module(
    ...
    env = jinja2.Environment(
        loader=jinja2.FileSystemLoader("tasks/app/templates/crud_module", autoescape=True)
    )
    ...
    template.stream(
        ...
        year=datetime.date.today().year,
        author=author,
    ).dump("%s/%s.py" % (admin_views_path, module_name))
